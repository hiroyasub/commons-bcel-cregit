...
begin_comment
comment|/**  * Dynamically creates and uses a proxy for<tt>java.awt.event.ActionListener</tt>  * via the classloader mechanism if called with  *<pre>java org.apache.bcel.util.JavaWrapper ProxyCreator</pre>  *  * The trick is to encode the byte code we need into the class name  * using the Utility.encode() method. This will result however in big  * ugly class name, so for many cases it will be more sufficient to  * put some clever creation code into the class loader.<br> This is  * comparable to the mechanism provided via  *<tt>java.lang.reflect.Proxy</tt>, but much more flexible.  *  * @version $Id$  * @author<A HREF="mailto:m.dahm@gmx.de">M. Dahm</A>  * @see org.apache.bcel.util.JavaWrapper  * @see org.apache.bcel.util.ClassLoader  * @see Utility  */
end_comment
