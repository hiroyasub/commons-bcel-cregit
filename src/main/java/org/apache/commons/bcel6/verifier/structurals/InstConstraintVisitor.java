...
begin_comment
comment|/**  * A Visitor class testing for valid preconditions of JVM instructions.  * The instance of this class will throw a StructuralCodeConstraintException  * instance if an instruction is visitXXX()ed which has preconditions that are  * not satisfied.  * TODO: Currently, the JVM's behaviour concerning monitors (MONITORENTER,  * MONITOREXIT) is not modeled in JustIce.  *  * @version $Id$  * @author Enver Haase  * @see org.apache.commons.bcel6.verifier.exc.StructuralCodeConstraintException  * @see org.apache.commons.bcel6.verifier.exc.LinkingConstraintException  */
end_comment
...
expr_stmt|;
block|}
comment|/**      * Assures index is of type INT.      * @throws org.apache.commons.bcel6.verifier.exc.StructuralCodeConstraintException if the above constraint is not satisfied.      */
specifier|private
name|void
name|indexOfInt
parameter_list|(
...
block|}
block|}
comment|/**      * Assures the ReferenceType r is initialized (or Type.NULL).      * Formally, this means (!(r instanceof UninitializedObjectType)), because      * there are no uninitialized array types.      * @throws org.apache.commons.bcel6.verifier.exc.StructuralCodeConstraintException if the above constraint is not satisfied.      */
specifier|private
name|void
...
block|}
block|}
comment|/**      * Assures arrayref is of ArrayType or NULL;      * returns true if and only if arrayref is non-NULL.      * @throws org.apache.commons.bcel6.verifier.exc.StructuralCodeConstraintException if the above constraint is violated.       */
specifier|private
name|boolean
